...
public class PeakTester {
    ...
    public static boolean hasPeak(LinkedList<Integer> list) {
        ...
        ListIterator<Integer> headIter = list.listIterator();
        ListIterator<Integer> tailIter = list.listIterator(list.size());

        // trace from head
        Integer headIndex = -1;
        Integer pre = 0;
        while (headIter.hasNext()) {
            Integer cur = headIter.next();
            if (headIter.previousIndex() == 0 || pre < cur) {
                headIndex++;
                pre = cur;
            } else {
                break;
        ...
        // trace from tail
        Integer tailIndex = list.size();
        while (tailIter.hasPrevious()) {
            Integer cur = tailIter.previous();
            if (tailIter.nextIndex() == list.size() - 1 || cur > pre) {
                tailIndex--;
                pre = cur;
            } else {
                break;
